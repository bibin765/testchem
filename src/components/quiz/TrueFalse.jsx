...
export default function TrueFalse({ questions, onComplete, onBack }) {
      ...
      >
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-blue-400 mb-4">True/False Quiz Results</h2>
                    ...
                    <div className="flex gap-4 mb-4">
                      <div className={`px-4 py-2 rounded-lg ${
                        result.correctAnswer === true
                          ...
                          : result.userAnswer === true && result.correctAnswer === false
                          ? 'bg-red-900 border border-red-500 text-red-100'
                          : 'bg-gray-700'
                      }`}>
                        True
                        {result.correctAnswer === true && <span className="ml-2 text-green-400">(Correct)</span>}
                        {result.userAnswer === true && result.correctAnswer === false && <span className="ml-2 text-red-400">(Your Answer)</span>}
                      </div>
                      <div className={`px-4 py-2 rounded-lg ${
                        result.correctAnswer === false
                          ? 'bg-green-900 border border-green-500 text-green-100'
                          : result.userAnswer === false && result.correctAnswer === true
                          ? 'bg-red-900 border border-red-500 text-red-100'
                          : 'bg-gray-700'
                      }`}>
                        False
                        {result.correctAnswer === false && <span className="ml-2 text-green-400">(Correct)</span>}
                        {result.userAnswer === false && result.correctAnswer === true && <span className="ml-2 text-red-400">(Your Answer)</span>}
                      </div>
                    </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-6 justify-center">
                <button
                  ...
                  className={`px-8 py-4 rounded-lg border transition-all text-lg font-semibold min-w-[120px] ${
                ...
                >
                  <div className="flex items-center justify-center gap-2">
                    <div className={`w-5 h-5 rounded-full border-2 ${
                      ...
                      {selectedAnswers[currentQuestion.id] === true && (
                        <div className="w-2 h-2 bg-green-600 rounded-full m-0.5" />
                      )}
                    </div>
                    TRUE
                  </div>
                ...
                >
                  <div className="flex items-center justify-center gap-2">
                    <div className={`w-5 h-5 rounded-full border-2 ${
                      ...
                      {selectedAnswers[currentQuestion.id] === false && (
                        <div className="w-2 h-2 bg-red-600 rounded-full m-0.5" />
                      )}
                    </div>
                    FALSE
                  </div>
